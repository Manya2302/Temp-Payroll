import { 
  ...
  type Payroll, type InsertPayroll, type LeaveRequest, type InsertLeaveRequest,
...
export class DatabaseStorage implements IStorage {
  ...
  async updateEmployee(id: string, updateEmployee: Partial<InsertEmployee>): Promise<Employee> {
    const [employee] = await db
      .update(employees)
  ...
  async updatePayroll(id: string, updatePayroll: Partial<InsertPayroll>): Promise<Payroll> {
    const [payroll] = await db
      .update(payrolls)
      .set(updatePayroll)
  ...
  async updateLeaveRequest(id: string, updateRequest: Partial<LeaveRequest>): Promise<LeaveRequest> {
    const [request] = await db
  ...
  async getDashboardStats(): Promise<any> {
    ...
    const [totalPayroll] = await db.select({ 
      total: sql<number>`COALESCE(sum(${payrolls.netSalary}), 0)` 
    }).from(payrolls);
